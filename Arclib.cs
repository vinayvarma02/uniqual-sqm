...
namespace CenturyLink_ArcApp
{
    public static class Arclib
    {
        ...
        public static IFeatureClass DoConvertPolygontoline(IFeatureClass inputShape, string outpupath)
        {
            ...
            try
            {
                ...
                PolygonToLine FeatureToLine = new PolygonToLine();
                FeatureToLine.in_features = path + "\\" + inputShape.AliasName + ".shp";
                // FeatureToLine.out_feature_class = tempfilesPath + name + ".shp";
                FeatureToLine.out_feature_class = outpupath;
                FeatureToLine.neighbor_option = "IDENTIFY_NEIGHBORS";
                IGeoProcessorResult result4 = (IGeoProcessorResult)gp.Execute(FeatureToLine, null);
                featClass = Getfeatureclass(outpupath);
            }
            catch (Exception ex)
            {
                LogManager.WriteLogandConsole(ex);
        ...
        public static IFeatureClass AddSpatialindex(IFeatureClass inputShape)
        {
            ...
            try
            {
                ...
                string path = pset.Workspace.PathName;
                gp.OverwriteOutput = true;
                AddSpatialIndex spatialIndex = new AddSpatialIndex();
                spatialIndex.in_features = path + "\\" + inputShape.AliasName + ".shp";
                IGeoProcessorResult result4 = (IGeoProcessorResult)gp.Execute(spatialIndex, null);
                fc = Getfeatureclass(path + "\\" + inputShape.AliasName + ".shp");
            }
            catch (Exception ex)
            {
                LogManager.WriteLogandConsole(ex);
        ...
        public static void selectbbyLocationtool()
        {
            try
            {
                ...
                gp.OverwriteOutput = true;
                SelectLayerByLocation sl = new SelectLayerByLocation();
                sl.in_layer = @"C:\SQM_Logs\sss.shp";
                //sl.out_layer_or_view = @"C:\SQM_Logs\sssoutput.shp";
                sl.overlap_type = "INTERSECT";
                sl.selection_type = "NEW_SELECTION";
                //sl.select_features = @"C:\SQM_Logs\roadbuffer.shp";
                IGeoProcessorResult result4 = (IGeoProcessorResult)gp.Execute(sl, null);
            }
            catch (Exception ex)
            {
        ...
        public static void Intersecting(string input, string output)
        {
        ...
        public static IFeatureClass DoConvertFeaturetopoint(string input, string output)
        {
            IFeatureClass featClass = null;
            try
            {
                gp.OverwriteOutput = true;
                FeatureToPoint FeatureToLine = new FeatureToPoint();
                FeatureToLine.in_features = input;
                FeatureToLine.out_feature_class = output;
                IGeoProcessorResult result4 = (IGeoProcessorResult)gp.Execute(FeatureToLine, null);
                featClass = Getfeatureclass(output);
        ...
        public static IFeatureClass Merge(string file1, string file2, string file3)
        {
            IFeatureClass outFC = null;
            string output = string.Empty;
            StringBuilder sb = new StringBuilder();
            sb.Append("'" + file1 + "'");
            sb.Append(";"); sb.Append("'" + file2 + "'"); ; sb.Append(";"); sb.Append("'" + file3 + "'"); ;
            try
            {
                // Initialize the geoprocessor.
                gp.OverwriteOutput = true;
                ...
                merge.output = tempfilesPath + "roadpoly.shp";
                IGeoProcessorResult result4 = (IGeoProcessorResult)gp.Execute(merge, null);
                IFeatureClass ifc = Getfeatureclass(merge.output.ToString());
                outFC = DoConvertlinetopolygon(ifc, "Mergedroadpolygons");
            }
            catch (Exception ex)
            {
        ...
        public static IGeometry Getbuffergeom(IGeometry geom)
        {
            ...
            try
            {
                topologicalOperator = geom as ITopologicalOperator;
                double convextoBufferVal = Convert.ToDouble(configKey.Default.BufferToConvex);
                buffergeo = topologicalOperator.Buffer(convextoBufferVal);
            ...
            finally { Marshal.ReleaseComObject(topologicalOperator); }
            return buffergeo;
        }
        ...
        public static void GpBuffer(string input, string output)
        {
            gp.OverwriteOutput = true;
            ESRI.ArcGIS.AnalysisTools.Buffer buffer = new ESRI.ArcGIS.AnalysisTools.Buffer();
            buffer.in_features = input;
            buffer.out_feature_class = output;
            buffer.buffer_distance_or_field = 0.0002;
            buffer.line_side = "FULL";
            buffer.line_end_type = "FLAT";
            IGeoProcessorResult result4 = (IGeoProcessorResult)gp.Execute(buffer, null);
        }
        ...
        public static IGeometry GetGeoEnvelope(IEnvelope envelope)
        {
            ...
            ipo.AddPoint(upl); ipo.AddPoint(upr); ipo.AddPoint(lr); ipo.AddPoint(ll);ipo.AddPoint(upl);
            return ipo as IGeometry;
        }
        ...
        public static void InsertCopperintoODS(IFeatureClass featureClass, List<CopperPolygonClass> geometryList)
        {
            try
            {
                ...
                // int objindex = featureClass.FindField("OBJECTID");
                int typeIndex = featureClass.FindField("QUAL_TYPE");
                int parceltypeindex = featureClass.FindField("PARCEL_TYPE");
                int statusindex = featureClass.FindField("QUAL_STATUS");
                int availDateIndex = featureClass.FindField("AVAIL_DT");
                int bandwidthindex = featureClass.FindField("BANDWIDTH");
                int clliindex = featureClass.FindField("WC_CLLI");
                int wcindex = featureClass.FindField("WC_NAME");
                int cableid = featureClass.FindField("CABLE_ID");
                int copperindex = featureClass.FindField("COPPER_LOC");
                int ndsindex = featureClass.FindField("NDS_JNO");
                int oltindex = featureClass.FindField("OLT_REL");
                int creationmodeindex = featureClass.FindField("CREATE_MODE");
                int apnindex = featureClass.FindField("APN");
                int fipsindex = featureClass.FindField("FIPS");
                int parcelindex = featureClass.FindField("PARCEL_ID");
                int latindex = featureClass.FindField("LU_LAT");
                int lonindex = featureClass.FindField("LU_LON");

                // Create a feature buffer.
                IFeatureBuffer featureBuffer = featureClass.CreateFeatureBuffer();
                IFeatureCursor featureCursor = featureClass.Insert(true);

                foreach (CopperPolygonClass sqmModel in geometryList)
                {
                    try
                    {
                        featureBuffer.set_Value(typeIndex, 3);
                        featureBuffer.set_Value(statusindex, 1);
                        featureBuffer.set_Value(apnindex, sqmModel.APN);
                        featureBuffer.set_Value(fipsindex, sqmModel.FIPS);
                        featureBuffer.set_Value(clliindex, sqmModel.WC_CLLI);
                        featureBuffer.set_Value(wcindex, sqmModel.WC_NAME);
                        featureBuffer.set_Value(creationmodeindex, 1);
                        featureBuffer.set_Value(bandwidthindex,sqmModel.Bandwidth);
                        featureBuffer.set_Value(latindex, sqmModel.LU_Lat);
                        featureBuffer.set_Value(lonindex, sqmModel.LU_Lon);
                        featureBuffer.set_Value(availDateIndex, sqmModel.AVAILABILITY_DATE);
                        featureBuffer.set_Value(parcelindex, sqmModel.Parcel_ID);
                        featureBuffer.set_Value(parceltypeindex,sqmModel.Parcel_Type);
        ...
        public static void InsertUpdateFeaturestoODS(IFeatureClass featureClass, List<ServicePolygonClass> geometryList)
        {
            ...
            try
            {
                ...
                string fcname = featureClass.AliasName;
        ...
        public static void InsertNobuildFeaturesFinal(IFeatureClass featureClass, List<NoBuild> geometryList, int createmode)
        {
            try
            {
                ...
                int dateti = featureClass.FindField("LST_UPD_DT");
                // Create an insert cursor with buffering enabled.
                string name = featureClass.AliasName;
                IFeatureCursor featureCursor = featureClass.Insert(true);
                foreach (NoBuild objNobuild in geometryList)
                {
                    try
                    {
                        ...
                        db.RunDmlQuery("delete from " + configKey.Default.Nobuild + " where ADDR_ID=" + objNobuild.Addr_id + "");
                        featureBuffer.set_Value(status, Convert.ToInt16(objNobuild.STATUS));
                        featureBuffer.set_Value(reason, objNobuild.REASON);
                        featureBuffer.set_Value(wcid, objNobuild.WC_ID);
                        featureBuffer.set_Value(addr_id, objNobuild.Addr_id);
